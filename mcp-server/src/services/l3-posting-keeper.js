...
export class L3PostingKeeperService {
  ...
  async enqueue({ termsHash } = {}) {
    ...
    return this.#withLock(`l3-posting-borrower:${borrower.toLowerCase()}`, async () => {
      ...
      const keeperLimitRaw = receiptGraphLimitRaw < L3_POSTING_LIMIT_RAW
        ? receiptGraphLimitRaw
      ...
      const effectiveHeadroomRaw = keeperHeadroomRaw < liveHeadroomRaw
        ? keeperHeadroomRaw
        : liveHeadroomRaw;
      if (BigInt(info.underwriting.trailingNetRaw ?? 0) === 0n || keeperLimitRaw === 0n) {
        return this.#refuse(
  ...
  async reconcile(id) {
    ...
    return this.#withLock(`l3-posting-borrower:${initial.borrower.toLowerCase()}`, async () => {
      ...
      if (Number(liveJob.state) === ESCROW_STATE_CANCELLED) {
        return this.#completeRefund(record, { recoveredCancellation: true });
      }
  ...
  async #completeRefund(record, { recoveredCancellation = false } = {}) {
    let updated = record;
    ...
    if (
      info.book.accountedLiquidity.raw !== updated.bookBefore.accountedLiquidityRaw
      || info.book.liquid.raw !== updated.bookBefore.liquidRaw
    ) {
      throw new ConflictError(
        ...
        {
          requestId: updated.id,
          expectedAccountedRaw: updated.bookBefore.accountedLiquidityRaw,
          observedAccountedRaw: info.book.accountedLiquidity.raw,
          expectedLiquidRaw: updated.bookBefore.liquidRaw,
  ...
  async #poll() {
    ...
    // Polling is always installed; the chain flag alone decides whether the
    // dormant keeper does work. No backend enablement switch exists.
  ...
  async #requireEnabled(entryPoint) {
    if (!this.chainReader) {
      throw new ConfigError("L3 posting chain reader is unavailable.");
    }
  ...
  async #withLock(lockId, operation, {
    ...
    if (!acquired) {
      return this.#refuse(busyReason, busyMessage, busyDetails);
    }
    try {
  ...
  async #refuse(reason, message, details = {}) {
    const refusedAt = this.#timestamp();
    const refusal = {
      id: randomUUID(),
