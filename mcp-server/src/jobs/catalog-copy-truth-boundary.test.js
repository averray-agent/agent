...
test("active ingestion and bundle titles describe audit reports rather than upstream effects", async () => {
  const generatedJobs = [
    githubIssueJob({
...
test("GitHub and OSV ingestion stop at an Averray report until delivery exists", () => {
  const jobs = [
    githubIssueJob({
  ...
  for (const job of jobs) {
    ...
    assert.match(job.agentInstructions.join(" "), /Do not modify the upstream repository or open a pull request/u);
    assert.doesNotMatch(job.verification.signals.join(" "), /pr_opened|merged|dependency_updated/u);
  }
});
