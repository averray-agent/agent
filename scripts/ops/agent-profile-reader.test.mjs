...
const [agentSource, helperSource, profileFixture, profileHtml] = await Promise.all([
...
test("real tester-wallet fixture renders the agent name, badge, and stats", async () => {
  let requestedUrl;
  ...
  assert.equal(elements.get("profile-tier").textContent, "PRO");
