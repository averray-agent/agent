...
const TEMPLATES = [
  ...
  // Mainnet profile — generated by scripts/ops/render-mainnet-backend-env.mjs
  // (drift-checked there via --check). Linted here so the mainnet op:// refs get
  // the same 3-segment / known-vault / inventory-row guarantees.
  { runtime: 'backend', path: 'deploy/backend.mainnet.env.template' },
  { runtime: 'indexer', path: 'deploy/indexer.mainnet.env.template' },
];
...
const KNOWN_VAULTS = new Set([
  ...
  'testnet-smoke',
  'testnet-critical',
  // mainnet tier — created by scripts/ops/bootstrap-mainnet-vault.mjs
  'mainnet-backend',
  'mainnet-backend-external',
  'mainnet-indexer',
  'mainnet-ci',
  'mainnet-ci-external',
  'mainnet-smoke',
...
const VAULT_TO_RUNTIME = {
  ...
  'prod-ci-external': ['ci'],
  'prod-smoke': ['smoke'],
  'mainnet-backend': ['backend'],
  'mainnet-backend-external': ['backend'],
  'mainnet-observability': ['backend'],
  'mainnet-indexer': ['indexer'],
  'mainnet-ci': ['ci'],
  'mainnet-ci-external': ['ci'],
  'mainnet-smoke': ['smoke'],
};
