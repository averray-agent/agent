...
test("an unreadable feed reports an em dash, never a confident zero", () => {
  ...
  for (const presence of ["unavailable", "locked", "loading"]) {
    ...
    assert.equal(tile.value, "—", `${presence} must not state a value`);
...
test("the refusal says which feed failed and that allocation is unobservable", () => {
  assert.match(
    ...
    /strategy feed unreachable — retrying — allocation not observable/u
  );
  assert.match(
    buildCapitalAtWorkVital({ presence: "locked", value: "0" }).delta,
    /unreachable — retrying/u
  );
