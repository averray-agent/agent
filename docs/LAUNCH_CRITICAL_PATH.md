...
| **Med** | Open prod dep advisories | `ws` via `ethers`/`viem` remediated with controlled chain-lib bumps plus a root `ws@8.21.0` override. Remaining: `drizzle`/`kysely` = **H3** (partially fixed, #686; residual inside Ponder), plus `vite` (frontend/Ponder transitive advisories). |
