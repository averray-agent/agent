...
| Expose settle/payout tx + verification-latency state | **DONE (both halves).** *Latency:* `/verifier/result` is session-aware — a submitted/disputed job without a verdict returns `{ status: "verifying", awaitingSince }` instead of an indistinguishable `not_found` (#639). *Payout tx:* `resolveSinglePayout` now returns its receipt `{ txHash, blockNumber, status }` (additive, mirrors `openDispute`/`resolveDispute`), and `verifySubmission` surfaces that `payoutTx` on **both** the verification result (so it lands in `/verifier/result` where the worker polls) and the session record (so it shows in `/session`). |
...
| Discovery reflects funding/settlement readiness | **DONE (code):** the original `ingestion_prefund` gate from #635 remains intact. Lazy-funded mainnet jobs with no funding metadata also use the shared cached AgentAccountCore reward-bank reading: insufficient liquidity reports `claimable: false` / `reward_funding_pending`, while an unreadable reading is tolerated only inside the last-known-good TTL and then reports `reward_funding_unverified`. Recurring-reserve jobs remain governed by their dedicated reserve. |
| Don't burn claims before delivery | **DONE:** `retryLimit` protects against delivered submissions and terminal submit-revert loops, not claim leases. A no-show claim that expires without a submit attempt returns to inventory without consuming the budget. A live claim whose on-chain submit fails remains recoverable; if that lease later expires, its `submitFailedAt` records a genuine failed mutation and consumes an attempt. Rejected submissions also consume an attempt. Claim fees and stake remain governed separately by the timeout/slashing policy. |
