...
| Postgres backup readiness | Ready for proof | `Hosted Backup Readiness Proof` runs `check-backup-readiness.sh --json` against the VPS, validates the saved JSON with `check-backup-readiness-evidence.mjs --max-checked-age-hours`, and uploads a 90-day artifact. If the VPS has no snapshots yet, run `Hosted Backup Snapshot Proof` first; it creates Postgres + Redis snapshots using the existing VPS scripts, then runs the same readiness validator. Close after one production run reports `overallStatus: "ok"` with the Postgres component `ok` and fresh. |
| Redis backup readiness | Ready for proof | Same hosted proof as Postgres. If the VPS has no snapshots yet, use `Hosted Backup Snapshot Proof` to create a Redis RDB snapshot before validation. Close after one production run reports `overallStatus: "ok"` with the Redis component `ok` and fresh, or records that Redis contains no non-rebuildable state and updates the checker accordingly. |
