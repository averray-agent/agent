...
| Verifier replay hardening | Done | `verifier-contract.js` now emits `policyVersion` (rules) separately from `verifierConfigVersion` (config-shape); legacy stores without `policyVersion` fall back to `version` so pre-split data still replays. Per-handler version map `HANDLER_VERSIONS` in `verifier-handlers.js` replaced the single `HANDLER_VERSION` constant — each handler declares its own version, and `detectReplayDrift` flags `policyVersion` drift alongside `handlerVersion`. Fourth handler (`human_fallback`) gained its first v1 fixture, and the fixture-coverage lint test (`fixture coverage: <handler> v<N> has at least one replay fixture`) asserts every entry in `HANDLER_VERSIONS` has at least one `<handler>/v<N>/<case>.json` whose `handlerVersion` matches — a v2 handler bump cannot ship without committing the matching v2 fixture in the same PR. Tests: 48 verifier/platform/ingestion cases pass; full `npm --workspace mcp-server test` is 819/819. |
