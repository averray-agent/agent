...
| Schema registration for external jobs | Open | First slice adds signed external schema registrations on job definitions: EVM signer recovery, explicit trusted issuer policy, schema hash/URL/trust metadata in job contracts, and runtime validation for registered external output schemas. Close when the registration path is wired into the external job posting/admin API and hosted proof shows an off-platform schema can be registered and used end to end. |
