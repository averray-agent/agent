...
| Postgres backup readiness | Ready for proof | `Hosted Backup Readiness Proof` runs `check-backup-readiness.sh --json` against the VPS, validates the saved JSON with `check-backup-readiness-evidence.mjs --max-checked-age-hours`, and uploads a 90-day artifact. Close after one production run reports `overallStatus: "ok"` with the Postgres component `ok` and fresh. |
| Redis backup readiness | Ready for proof | Same hosted proof as Postgres. Close after one production run reports `overallStatus: "ok"` with the Redis component `ok` and fresh, or records that Redis contains no non-rebuildable state and updates the checker accordingly. |
