...
| Operator self-report evidence | Ready for proof | `Hermes Operator Report` workflow schedules and manually runs `ops_health` and `daily_operator_brief` through Hermes with correlation IDs, step summaries, and 90-day artifacts. Close after the first production run succeeds for both report kinds and hosted bootstrap instrumentation smoke still proves `/admin/status` is sanitized. |
