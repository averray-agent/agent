...
| Dispute verdict hosted proof | Ready for proof | `scripts/ops/check-hosted-stack.sh` now has an opt-in `CHECK_DISPUTE_VERDICT_PROOF=1` gate that runs `run-dispute-verdict-proof.mjs` with `DISPUTE_PROOF_LIVE=1`, requires chain dispatch (`confirmed` or `submitted`, never `local_only`), asserts the persisted dispute matches the verdict response, and can write `DISPUTE_PROOF_EVIDENCE_FILE`. `Hosted Dispute Verdict Proof` wraps that gate in a production GitHub Actions workflow with explicit `live=true` confirmation and a sanitized `hosted-dispute-verdict-proof-<run-id>` artifact. Close after the operator runs it against a specific hosted open dispute and records the evidence artifact + tx hash. Polkadot docs MCP check: official Polkadot Hub smart-contract docs confirm REVM/Solidity contract support; no protocol semantics changed in this PR. |
