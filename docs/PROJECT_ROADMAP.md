...
| Metrics auth | Ready for proof | Code deployed in `#445`; hosted `/metrics` now fails closed with `503 metrics_auth_unconfigured` when no scraper token is configured. Close after production `METRICS_BEARER_TOKEN` is set and the `Hosted Observability Proof` workflow proves unauthenticated `401` plus scraper-token `200` against the hosted stack. Combined evidence for this row, Sentry/logging, and Alert destination lands in one sanitized artifact validated by `node scripts/ops/check-observability-proof.mjs --file ... --json` (`#481`). |
| Sentry/logging decision | Ready for proof | Backend/frontend Sentry posture is recorded in `OBSERVABILITY_POSTURE.md`; backend 5xx capture falls back to structured JSON logs and optional Sentry has regression coverage. Close after the `Hosted Observability Proof` workflow samples structured backend logs from the active deploy target, and `observability.sentry_ready` is observed if Sentry is enabled. Evidence shape validated by `check-observability-proof.mjs` (`#481`) — operator records the decision + log-visibility check in the same observability artifact as Metrics auth and Alert destination. |
| Alert destination | Ready for proof | Alert wrapper is tested for structured webhook delivery on smoke failure in `#449`; it now supports a channel-visible `correlationId` for providers that do not return a native message timestamp. Close after `ALERT_WEBHOOK_URL` is configured in the production runtime/scheduler environment and the `Hosted Observability Proof` workflow sends one deliberate hosted smoke failure to the operator channel. Captured in the observability evidence artifact (`#481`) alongside metrics auth and Sentry decision. |
...
| A2 — Receipts and badges metric clarity | Open | Frontend | receipts / reputation trust surface | Top receipt metric stops reading as contradictory with the Agents badge count by showing receipt-type breakdown (`run`, `badge`, `settle`, `policy`) or another approved label. | Operator app screenshot or UI test proving badges and receipt shapes are differentiated at first glance. |
...
| A4 — Overview first-load orientation | Open | Frontend | operator onboarding / reputation deepening | First-time or low-activity operators see a dismissable next-step card that explains what the room is and points to the first useful action without competing with the vitals. | Browser screenshot/state fixture proving the card appears for early-state operators and disappears or can be dismissed after activity/threshold. |
| A5 — Sidebar count badge consistency | Open | Design steward / frontend | sidebar polish | Decide count convention, then apply consistently: show meaningful counts across Disputes/Policies/Audit where useful, or remove counts entirely. | Design decision note plus screenshot across sidebar surfaces. |
| A6 — Provider Operations operator-language pass | Open | Frontend | overview comprehension | Overview provider-operation rows explain or relabel `candidate`, `created`, `skipped`, and `error` into operator-readable language. | Browser screenshot or component test proving the legend/labels are present. |
...
| C1 — Chain explorer link on chain-anchored entities | Open | Frontend | trust-surface / reputation deepening | Every page that displays a chain-anchored tx/event/receipt/dispute/capability grant exposes a small explorer link that lands on the correct environment-specific explorer view. | UI check for every affected page, plus at least one testnet and one mainnet-style URL fixture. |
...
| C6 — Agent directory to public profile bridge | Open | Frontend | reputation deepening / trust-surface | Every agent row links to `averray.com/agent/<wallet>` and the public profile renders the same reputation/badge data as the internal row. | Browser check from internal row to public profile plus data-consistency assertion. |
...
2. Run the `Hosted Dispute Verdict Proof` workflow against a specific open
   dispute and record the evidence artifact plus chain dispatch status.
