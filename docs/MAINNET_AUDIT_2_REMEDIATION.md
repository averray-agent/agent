...
| **L-6** | XcmWrapper binds only `keccak256(destination/message)`; instruction skippers never decode asset/amount/beneficiary vs `context` → mismatched XCM payload still dispatches | `XcmWrapper.sol` `queueRequest`:94-143 (:106-107); `_validateXcmPayload`; `_decodeWithdrawAssetAndAssert`; `_decodeDepositAssetAndAssert` | new | **In review; auditor re-verification pending.** The vDOT launch message shape is now lockstep with `mcp-server/src/blockchain/xcm-message-builder.js`: `WithdrawAsset` must contain exactly one fungible relay-native asset encoded as `Location { parents: 1, interior: Here }`; its amount must equal `context.assets` (or `context.shares` for share-denominated withdraw contexts where `assets == 0`). `DepositAsset` must use canonical `Wild(AllCounted(1))` bytes (`0x01 0x02 0x04`) and beneficiary `Location { parents: 0, interior: X1(AccountKey20 { network: None, key: context.recipient }) }`. Mismatches revert before dispatch. |
