...
| T7 | **Drafts bind by `specHash`** (canonical-content hash of the definition); funding calldata must carry it exactly; drafts expire (TTL) and unmatched funding is refundable via the mismatch path. | Watcher matched in ~162s |
