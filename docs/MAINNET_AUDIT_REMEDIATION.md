...
| **C-18** | `workerClaimCount` never decrements → workers permanently penalized for timeouts | `EscrowCore.sol` `handleClaimTimeout` | Codex | ✅ remediated → **audit-2 L-3** (severity downgraded High→Low); `handleClaimTimeout` restores one onboarding-waiver slot for claimed-but-never-submitted timeouts; auditor re-verification pending |
