...
| D-03 | CI auto-deploy without manual gate | Pascal/infra → Codex | 🟢 Semantic two-tier deploy gate: every enabled deploy verifies live runtime hashes against the selected manifest (real mismatch fails, never sticky); only a `contracts/**` change whose immutable-masked compiled runtime differs from deployed provenance can create the sticky freeze. Exact known-unshipped runtimes require a manifest hash and reason. `allow_contract_surface_drift=1` remains dispatch-only; unreadable manifests and unreachable RPCs always fail closed. A broad GitHub "required reviewers" human gate remains separate optional hardening. |
