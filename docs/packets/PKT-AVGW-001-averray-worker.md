...
- `spec.acceptance` — **discriminated union on `type`**. Derive DETERMINISTIC checks from a verify command (`options.verifyCommand ?? job.verification.suggestedCheck`):
  - with a command: `[{id:"job-checks", type:"command", command:<cmd>, required:true}]`, plus — **only when the command invokes pytest** — `{id:"no-regressions", type:"baseline_comparison", rule:"no_new_failures", baseline_command:<cmd>, required:true}`.
  - **ERRATUM (live-smoke finding, 2026-07-22 — supersedes the original unconditional baseline check):** the kernel captures `no_new_failures` baselines ONLY for pytest — it appends `--junitxml` and parses JUnit failure identities, and its contract compiler rejects any other `baseline_command` at run time (`contract_compilation_failed: invalid_baseline_command: baseline_command must invoke pytest`; `harness validate` does NOT catch this, it is schema-only). Mirror the kernel predicate (`verification/checks.py pytest_arguments`): executable is `pytest`/`pytest-*`, or `python|python3|python3.x -m pytest`, and the command does not already set `--junitxml`. For every other verify command (e.g. `npm test`) the command check is the sole deterministic gate.
...
- **Contract:** generate an intent from a sample GitHub-issue job and run `harness validate` on it → exit 0 (the harness echoes `profile=averray-worker acceptance=job-checks` — plus `,no-regressions` only for a pytest-invoking verify command, per the §3 erratum).
