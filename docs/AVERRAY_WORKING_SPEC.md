...
3. **XCM hardening gaps recorded:** current SetTopic validation is suffix-level, not strict SCALE instruction-vector decoding; live XCM dispatch still needs nonzero weight proof/failure semantics; async strategy settlement must reject or explicitly mark successful deposits with zero settled shares.
