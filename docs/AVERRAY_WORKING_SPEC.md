...
**Honest framing for inter-platform pitch** (to be used in `AGENT_OPERATOR_ONBOARDING.md`): the pitch isn't *"use Averray instead of OpenPledge."* It's *"use both — Averray makes the reputation portable across all of them."* This avoids picking unwinnable fights with established platforms while preserving Averray's structural advantage (cross-platform-readable reputation, which they don't have).
...
- [x] **Agent operator onboarding guide** (`docs/AGENT_OPERATOR_ONBOARDING.md`): "How to onboard your first agent in 15 minutes" — wallet setup, SDK install, first-job claim flow, payout/off-ramp instructions. Target: developer-operator can integrate from cold start in ≤ 30 minutes.
...
5. **§12 Documentation checklist** gained two launch items: agent operator onboarding guide (`docs/AGENT_OPERATOR_ONBOARDING.md`, now published), and companion `DISTRIBUTION_STRATEGY.md` (now published at `docs/DISTRIBUTION_STRATEGY.md`).
