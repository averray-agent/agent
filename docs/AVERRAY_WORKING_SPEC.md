...
3. **XCM hardening gaps narrowed:** `XcmWrapper` now rejects `SetTopic` bytes that sit outside the declared XCM instruction vector and requires `SetTopic(requestId)` to be the final declared instruction for the supported v5 message subset. Backend XCM weight handling preserves exact `uint64` values, and observer/finalize settlement handling preserves exact `uint256` base-unit amounts instead of coercing through JavaScript `Number`. Live XCM dispatch still needs precompile execution/failure semantics.
