...
import {DepositPoolV22} from "../DepositPoolV22.sol";
import {IStrategyAdapter} from "../interfaces/IStrategyAdapter.sol";
import {ReentrancyGuard} from "../lib/ReentrancyGuard.sol";
import {SafeTransfer} from "../lib/SafeTransfer.sol";

interface IERC20AggregatorV22Asset {
    function balanceOf(address account) external view returns (uint256);
}

/// @title AAC committed-balance DepositPool v2.2 aggregator
/// @notice Synchronous AAC strategy adapter that nets consented locked balances in
...
contract AacPoolAggregatorAdapterV22 is IStrategyAdapter, ReentrancyGuard {
    bytes32 public constant override strategyId = bytes32("AAC_LOCKED_DEPOSIT_POOL_V22");

    address public immutable agentAccountCore;
    DepositPoolV22 public immutable pool;
    ...
    event FloatExitRequested(uint256 indexed requestId, uint256 poolShares, DepositPoolV22.NoticeTier tier);
    ...
    constructor(address agentAccountCore_, DepositPoolV22 pool_) {
    ...
    function riskLabel() external pure override returns (string memory) {
        return "Shared committed AAC USDC in DepositPool v2.2; one pro-rata NAV including venue gain or loss. Pool shares cannot exit before the shared commitment expires; subsequent notice and venue recall may delay release. Direct strategy allocation does not bypass the existing shared commitment. Principal is at risk.";
    }

    /// @notice Recommit the shared position only up to the keeper's freshly
    /// reconciled earliest consent. Incompatible newcomers stay idle in AAC.
    function commitSharedUntil(uint64 consentUntil) external onlyOperator nonReentrant {
        _commitSharedUntil(consentUntil);
    }

    function _commitSharedUntil(uint64 consentUntil) private {
        uint256 remaining = consentUntil > block.timestamp ? consentUntil - block.timestamp : 0;
        if (remaining == 0 || remaining > 90 days) revert InvalidCommitment();
        DepositPoolV22.NoticeTier tier = remaining <= 7 days ? DepositPoolV22.NoticeTier.Notice7Days
            : remaining <= 30 days ? DepositPoolV22.NoticeTier.Notice30Days : DepositPoolV22.NoticeTier.Notice90Days;
        pool.commitUntil(tier, consentUntil);
    }

    error InvalidCommitment();

    function floatAssets() public view returns (uint256) {
        return IERC20AggregatorV22Asset(asset).balanceOf(address(this));
    }

    /// @notice Move uncommitted float into the adapter's own pool position.
    function sweepToPoolAndCommit(uint256 assets, uint64 consentUntil) external onlyOperator nonReentrant returns (uint256 poolShares) {
    ...
    function requestFloatExit(uint256 poolShares, DepositPoolV22.NoticeTier tier)
        external
        onlyOperator
        nonReentrant
