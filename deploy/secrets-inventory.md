...
| `admin-eoa-testnet`                      | `prod-critical` | Operator-side admin EOA for Paseo Asset Hub TestNet (`deployer` / `pauser` / `arbitrator` per `deployments/testnet.json`). Derives to `0x6778F050…3ac8`. Created 2026-05-25 to replace `0xFd2EAE…6519` after that key's value leaked into a Claude Code session transcript (see `SIGNER_PRIVATE_KEY` row above for the lineage). **Not loaded into any backend template** — backend signs chain mutations via KMS (see `KMS_KEY_ID` row). This item is consumed only by ad-hoc operator-side admin scripts (`scripts/ops/rotate-admin-*.mjs`, `scripts/ops/admin-topup-kms-signer.mjs`) via the in-process `loadKeyFromEnvFile` pattern — keys flow into `ethers.Wallet` via `fs.readFileSync`, never to stdout, stderr, or `process.env`. Also held locally in `mcp-server/.env.local` (gitignored, mode 0600) for the same scripts; the OP item is the canonical store. |
