...
| `X402_SETTLEMENT_API_KEY_ID`     | `op://prod-backend/x402-cdp-facilitator/username`   | ✅ yes            | deployer       | Coinbase Developer Platform API key **id** for the x402 settlement facilitator. Not a secret strictly — it is an identifier — but kept beside its secret so rotation moves both. Rotate at the CDP dashboard → API Keys; the key must be **Ed25519**, since the adapter signs `EdDSA` and decodes a raw Ed25519 secret. |
| `X402_SETTLEMENT_API_KEY_SECRET` | `op://prod-backend/x402-cdp-facilitator/credential` | ✅ yes            | deployer       | CDP API key **secret** that authorizes `/verify` and `/settle` against the facilitator. Shown once at creation. Granted **no optional scopes** — no Trade, Transfer, Receive, Export or Manage — because those govern the Coinbase App brokerage account, which settlement never touches. Prove a replacement works with `scripts/ops/check-x402-ramp-readiness.mjs` before arming: presence is not proof, and the pre-flight makes a live call. |
...
| `AWS_BADGE_RECEIPT_REGION` | `op://prod-backend/aws-badge-receipt-signer-testnet/aws-region` | ✅ yes | deployer | Region for the dedicated P-256 receipt key. Credentials come from Roles Anywhere profile `averray-badge-receipt-signer`; no static AWS credentials are rendered. |
| `AWS_BADGE_RECEIPT_KEY_ID` | `op://prod-backend/aws-badge-receipt-signer-testnet/kms-key-id` | ✅ yes | deployer | Full KMS key ARN only. Aliases are rejected at config load and compared with the ARN returned by `kms:GetPublicKey` at boot. |
| `BADGE_RECEIPT_PUBLIC_KEY_PEM_BASE64` | `op://prod-backend/aws-badge-receipt-signer-testnet/public-key-pem-base64` | ✅ yes | deployer | Base64 of the public P-256 SPKI PEM. Compared byte-for-byte with `kms:GetPublicKey` before the backend can start. |
...
| Env var | `op://` path | Critical-nonempty | Rotation owner | Notes |
| ------- | ------------ | :---------------: | -------------- | ----- |
| `KMS_KEY_ID` | `op://mainnet-backend/aws-signer-mainnet/kms-key-id` | see prod-* row | deployer | Mainnet mirror — generated; rotation detail on the matching prod-* row. |
| `AWS_REGION` | `op://mainnet-backend/aws-signer-mainnet/aws-region` | see prod-* row | deployer | Mainnet mirror — generated; rotation detail on the matching prod-* row. |
| `AWS_JWT_REGION` | `op://mainnet-backend/aws-jwt-signer-mainnet/aws-region` | see prod-* row | deployer | Mainnet mirror — generated; rotation detail on the matching prod-* row. |
