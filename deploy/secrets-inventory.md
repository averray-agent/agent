...
| `METRICS_BEARER_TOKEN` | `op://prod-backend/metrics-bearer-token/password` | ⚠️ not active in template | operator | High-entropy bearer token for the public `/metrics` scraper gate. Production intentionally returns `503 metrics_auth_unconfigured` when `METRICS_AUTH_REQUIRED=1` and this token is absent. Activate the template row only after the OP item exists, then run `Hosted Observability Proof` to prove unauthenticated `401` and authenticated `200` without writing the token to evidence. |
| `ALERT_WEBHOOK_URL` | `op://prod-backend/alert-webhook-url/url` | ⚠️ not active in template | operator | Slack Incoming Webhook for the operator alert channel. Consumed by the production-side `scripts/ops/check-hosted-stack-and-alert.sh` wrapper and the `Hosted Observability Proof` workflow; a failed hosted smoke POSTs structured JSON plus a channel-visible `correlationId` to this URL. Keep `deploy/backend.env.template` blank until the item exists, because `op inject` fails closed on missing optional items. Rotate by creating a replacement Slack webhook, updating this 1Password field, re-rendering the scheduler env, and re-running the deliberate smoke-failure proof. |
